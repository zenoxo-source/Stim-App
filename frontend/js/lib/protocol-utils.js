// protocol-utils.js - Pure V3 BLE protocol helpers (browser + Node tests)
 ...
 * @returns {number}
 */
export function getDeviceStrength(val, softLimit, masterScale) {
 ...
 * @returns {number}
 */
export function scaleWaveAmp(amp, masterScale) {
  const scale = typeof masterScale === "number" && !Number.isNaN(masterScale) ? masterScale : 1;
 ...
 * @returns {number}
 */
export function applyPulseWidthScale(amp, pulseWidth) {
 ...
 * @returns {number}
 */
export function encodeWaveFreqLogical(input) {
 ...
 * @returns {number}
 */
export function clampWireFreq(freq) {
 ...
 * @returns {string}
 */
export function waveFreqLabel(wire) {
 ...
 */
export function resolveWaveSegment(freq, amp) {
 ...
 */
export function fillChannelWave(data, freqOffset, intOffset, freq, intensity) {
  for (let i = 0; i < 4; i++) {
...
export function buildEmergencyStopBytes() {
 ...
 */
export function buildSoftStopBytes(opts) {
  const o = opts || {};
  const data = new Uint8Array(20);
  data[0] = 0xb0;
...
export function parseSettingsImport(raw) {
...
export function mergeHighscore(store, gameId, score) {
  const all = { ...(store || {}) };
